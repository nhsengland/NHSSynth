...
class ConditionalDatasetSampler(BaseSampler):
    ...
    def __init__(
        ...
        self._train_idx, self._test_idx = train_test_split(indices, train_size=train_size)
        self._train_mapping = {old_idx: new_idx for new_idx, old_idx in enumerate(self._train_idx)}
    ...
    def sample_conditional(self, batch: int, with_ids: bool = False, p: Optional[np.ndarray] = None) -> Any:
        ...
        category_id = self._categorical_feat_offset[discrete_column_id] + category_id_in_col
    ...
    def _internal_setup(
        ...
        def is_discrete_column(column_info) -> bool:
        ...
        max_category = max(
            [column_info.output_dimensions for column_info in output_info if is_discrete_column(column_info)],
            default=0,
        )
        ...
        for column_info in output_info:
            if is_discrete_column(column_info):
                ...
                self._discrete_feat_value_prob[current_id, : column_info.output_dimensions] = category_prob
                self._categorical_feat_offset[current_id] = current_cond_st
                self._categorical_feat_dimension[current_id] = column_info.output_dimensions
        ...
        self._conditional_probs = self._conditional_probs / (np.sum(self._conditional_probs) + 1e-8)
    ...
    def _prepare_dataset_conditionals(self) -> None:
        ...
        sampling_indices = self.sample_conditional_indices(categoricals, categoricals_vals)
