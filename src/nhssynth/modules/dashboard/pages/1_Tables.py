...
def prepare_evals(evaluations: pd.DataFrame, experiments: pd.DataFrame) -> pd.DataFrame:
    ...
    metrics_to_show = st.sidebar.multiselect(
        "Select metric(s) to display", valid_metrics, default=valid_metrics
    )
    # Filter to only the columns we want to display
    return evaluations[metrics_to_show].join(experiments)[
        experiments.columns.tolist() + metrics_to_show
    ]
...
def group_by_display(
    ...
    if group_by_cols:
        evaluations_shown = (
            evaluations_filtered[
                [
                    ...
                    if c not in experiments.columns.tolist() + experiments.index.names
                    or c in group_by_cols
                ]
            ]
        ...
        evaluations_shown.index = evaluations_shown.index.set_levels(
            evaluations_shown.index.levels[0], level=0
        )
...
def table_metrics(evaluations, experiments, selected_metric_group):
    ...
    group_by_display(
        prepared_evals, experiments, f"{selected_metric_group.capitalize()} metrics"
    )


def columnwise_metrics(evaluations, experiments):
    display = st.sidebar.selectbox(
        "Display", ["By column", "By metric", "By configuration"], index=0
    )
    ...
    if display == "By column":
        ...
        prepared_evals = prepare_evals(evaluations, experiments)
        group_by_display(
            prepared_evals, experiments, f"Columnwise metrics for `{column}`"
        )
        ...
        metric_evaluations = (
            evaluations[metric]
            .apply(lambda x: pd.Series(x).apply(lambda y: y["score"]))
        ...
        exploded_evaluations = config_evaluations.apply(
            lambda x: pd.Series(x).apply(lambda y: y["score"])
        ).T
        metrics_to_show = st.sidebar.multiselect(
...
def pairwise_metrics(evaluations, experiments):
    ...
    metric_column_map = {
        metric: set([item for pair in evaluations[metric][0].keys() for item in pair])
    ...
    for column in columns:
        for metric_columns in metric_column_map.values():
            if column in metric_columns:
                column_column_map[column] = (
                    column_column_map[column].union(metric_columns).difference({column})
                )
    if display == "By column pair":
        column1 = st.sidebar.selectbox(
            "Select first column to display", columns, index=0
        )
        column2 = st.sidebar.selectbox(
            "Select second column to display", column_column_map[column1], index=0
        )
        column_pair_evaluations = evaluations.apply(
            lambda x: x.apply(
                lambda y: (
                    y.get((column1, column2)) or y.get((column2, column1)) or {}
                ).get("score")
            )
        )
        ...
        reference_column = st.sidebar.selectbox(
            "Select reference column", columns, index=0
        )
        ...
        metric_evaluations.columns = [
            (col[0] + col[1]).replace(reference_column, "")
            for col in metric_evaluations.columns
        ]
        joined_evals = metric_evaluations.join(experiments)
        group_by_display(
            joined_evals,
