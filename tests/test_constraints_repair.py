...
def test_binary_relation_repair(meta):
    df = pd.DataFrame({"a": np.random.normal(0,1,5000),
                       "b": np.random.normal(0,1,5000)})
    # meta.constraints includes a <= b
    out = meta.repair_constraints(df)
    assert (out["a"] <= out["b"]).all()

def test_sum_relation_repair(meta):
    df = pd.DataFrame({"a": np.abs(np.random.gamma(2., 2., 5000)),
                       "b": np.abs(np.random.gamma(2., 2., 5000)),
                       "c": np.abs(np.random.gamma(2., 2., 5000))})
